...
const populatateUser = (ctx:QueryCtx,id:Id<"users">) =>{
...
export const get = query({
    ...
    if(!userId){
        //if we return null the whole app will crashed so we dont want
        // to crahsed and always want return an empty array
       return [];
    }
    const member = await ctx.db.query("members").withIndex("by_workspace_id_user_id",
        ...
        const user = await populatateUser(ctx, member.userId);
            if(user){
                members.push({
                    ...member,user
                })
