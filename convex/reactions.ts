...
export const toggle = mutation({
    ...
    const userId = await auth.getUserId(ctx);
     ...
     const existingMessageReactions =  await ctx.db.query("reactions")
        .filter((q) => q.and(
        ...
        if(existingMessageReactions){
            await ctx.db.delete(existingMessageReactions._id);
            return existingMessageReactions._id
        }else{
